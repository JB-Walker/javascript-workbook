...
function generateSolution (predeterminedSolution) {
  if (predeterminedSolution === false) {
    ...
    // Create string form of the array. Then remove all "," using regexp
...
function generateHint (guess) {
  ...
  //  guessAndSolutionArr is used to compare guess to solution.
  //  It is the concatenation of guess + solution in array form.
  //  As matches are found, the relevant elements are replaced with
  //  special symbols so that those elements can not be used in
  //  subsequent element comparisons. A single concatenated array
  //  also aids with the callback functions.
  let guessAndSolutionArr = guessArr.concat(solutionArr);
  const countInPosition = findInPosition();
  const countOutOfPosition = findOutOfPosition();
  ...
  // Determines remaining number of letters in the guess that match remaining
...
function mastermind (guessInput) {
  const guess = validateGuess(guessInput);
  if (guess !== false) {
    if (guess !== solution) {
